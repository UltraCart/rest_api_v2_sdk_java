...
|------------ | ------------- | ------------- | -------------|
|**cjsonSize** | **Integer** | Size of the CJSON in UTF-8 bytes, which is the same unit the container version size and the upsell offer cjson_size use. |  [optional] |
...
|**ownerType** | [**OwnerTypeEnum**](#OwnerTypeEnum) | Always item.  Carried so a row is usable as an address without the caller having to know which listing it came from. |  [optional] |
